...
async function trackEngagementEvent(ev) {
  ...
  try {
    ...
    const rawEventData = {
      ...
      deviceInfo,
      sessionTimeOnPage: sessionTime // Include session time in event data for reference
    };
    const insertRaw = `
  ...
  try {
    ...
    for (const e of (events.length ? events : [req.body])) {
      ...
      await trackEngagementEvent({
        ...
        ip: clientIP,
        sessionInfo: sessionInfo  // Pass sessionInfo to trackEngagementEvent
      });
    }
    ...
    if (sessionInfo?.inquiryId) {
      ...
      // CRITICAL FIX: Update the inquiries table with accumulated dwell time
