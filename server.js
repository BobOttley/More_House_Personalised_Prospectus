...
// Fixed generateProspectus function - EMBED tracking instead of external file
...
async function generateProspectus(inquiry) {
  try {
    ...
    // STEP 1: Create personalization script (unchanged)
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  if (typeof initializeProspectus === 'function') {
    ...
    // STEP 2: EMBED THE TRACKING SCRIPT DIRECTLY (NOT as external file)
    const embeddedTrackingScript = `<script>
/* More House School - Embedded Prospectus Tracking System */
(function() {
  'use strict';

  // Configuration
  const INQUIRY_ID = '${inquiry.id}';
  const SESSION_ID = 'S-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6);
  ...
  let readingBehaviorData = {
    readingSpeed: 0,
    attentionQuality: 'scanning',
  ...
  function detectCurrentSection() {
    ...
    // Method 2: Viewport-based fallback
    const scrollPercent = Math.round((window.pageYOffset / (document.documentElement.scrollHeight - window.innerHeight)) * 100);
    let sectionId = 'top';
    
    if (scrollPercent > 80) sectionId = 'bottom';
    else if (scrollPercent > 60) sectionId = 'lower';
    else if (scrollPercent > 40) sectionId = 'middle';
    else if (scrollPercent > 20) sectionId = 'upper';
  ...
  function getSectionLabel(sectionId) {
    const labels = {
      ...
      'middle': 'School Life',
      'lower': 'Facilities & Activities',
      'bottom': 'Admissions & Contact'
    };
  ...
  function track(eventType, data = {}) {
    ...
    if (['section_enter', 'video_open', 'conversion_signal'].includes(eventType)) {
      event.data.conversionSignal = 1;
  ...
  // Section management
  function enterSection(sectionData) {
    if (currentSectionId === sectionData.id) return;
    ...
    currentSectionEl = sectionData.element;
    
    if (!sectionState.has(sectionData.id)) {
      sectionState.set(sectionData.id, {
        id: sectionData.id,
        ...
        maxScrollPct: 0,
        clicks: 0,
        firstEnterAt: Date.now()
      });
    }

    const st = sectionState.get(sectionData.id);
    st.visitCount++;
    st.lastTickAt = Date.now();

    track('section_enter_enhanced', {
  ...
  function exitSection() {
    ...
    const st = sectionState.get(currentSectionId);
    if (st && st.lastTickAt) {
      const exitTime = Date.now();
      const timeInSection = Math.round((exitTime - st.lastTickAt) / 1000);
      st.attentionSec += timeInSection;

      track('section_exit_enhanced', {
        section: currentSectionId,
        label: st.label,
        timeInSectionSec: timeInSection,
        totalAttentionSec: st.attentionSec,
        visitCount: st.visitCount,
        scrollPct: st.maxScrollPct
      });
    }
  ...
  function sectionScrollPct(element) {
    if (!element) return 0;
    ...
    if (elementHeight <= viewportHeight) {
      return rect.top <= 0 && rect.bottom >= 0 ? 100 : 0;
    }
    
    const visibleTop = Math.max(0, -rect.top);
    const visibleHeight = Math.min(elementHeight - visibleTop, viewportHeight - Math.max(0, rect.top));
    return Math.round((visibleHeight / elementHeight) * 100);
  }

  // Heartbeat function
  function heartbeat() {
    if (currentSectionId && attentionActive) {
      const st = sectionState.get(currentSectionId);
      if (st && st.lastTickAt) {
        const now = Date.now();
        const deltaMs = now - st.lastTickAt;
        st.attentionSec += Math.max(0, Math.round(deltaMs / 1000));
        st.lastTickAt = now;
        st.maxScrollPct = Math.max(st.maxScrollPct, sectionScrollPct(currentSectionEl));
      }
    }
    
    const allEvents = highPriorityQueue.splice(0, highPriorityQueue.length)
    ...
    const sessionInfo = {
      ...
      maxScrollDepth: Math.max(0, ...Array.from(sectionState.values()).map(s => s.maxScrollPct)),
      clickCount: Array.from(sectionState.values()).reduce((a, b) => a + (b.clicks || 0), 0),
      sectionsExplored: Array.from(sectionState.keys()),
      currentSection: currentSectionId,
    ...
    const isRegularUpdate = (Date.now() - sessionStartTime) % (HEARTBEAT_MS * 3) < HEARTBEAT_MS;
    
    if (hasEvents || isRegularUpdate) {
      sendData(POST_URL, payload).catch(error => {
      ...
      const finalPayload = {
        events: [{
          ...
          data: {
            inquiryId: INQUIRY_ID,
            sessionId: SESSION_ID,
            totalTimeOnPage: Math.round((Date.now() - sessionStartTime) / 1000),
            sectionsVisited: Array.from(sectionState.keys())
          }
        }]
      };
      
      navigator.sendBeacon(POST_URL, JSON.stringify(finalPayload));
    });
  }
  ...
  function initialize() {
    ...
    setTimeout(() => {
      ...
      if (initialSection) {
        ...
        track('session_entry_context', {
          landingSection: initialSection.id,
          landingLabel: initialSection.label,
